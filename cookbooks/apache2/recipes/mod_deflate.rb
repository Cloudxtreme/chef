...
unless File.exists?("/usr/local/apache-#{node[:apache2][:version]}/modules/mod_deflate.so")
  extract "httpd-#{node[:apache2][:version]}" do
    action :enable
  end

  bash "build mod_deflate" do
    code <<-EOH   
      pushd "#{node[:common][:tmp_dir]}/httpd-#{node[:apache2][:version]}/modules/filters" >/dev/null
      /usr/local/apache-#{node[:apache2][:version]}/bin/apxs -c -i mod_deflate.c
      popd >/dev/null
    EOH
  end 
